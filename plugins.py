...
if platform.system() == 'Windows':
    sys.path.append('E:/MayaPython/MayaScripts')
else:
...
def showworkspace(scene):
    ...
    cmds.confirmDialog(t=u'项目路径', m=curlworkspace, b='OK')
...
def showPlugins(sence):
    if plugins == None:
        cmds.confirmDialog(t=u'提示', m=u'场景未使用插件', b='OK')
    else:
        cmds.confirmDialog(t=u'提示', m=",".join(plugins), b='OK')
...
def showFumeFX(fumeNodes):
    ...
    if 'ffxDyna' in cmds.ls(showType=True):
        ...
        cmds.confirmDialog(t=u'提示', m=",\r".join(AllPath), b='OK')
    else:
        cmds.confirmDialog(t=u'提示', m=u'未使用FumeFX插件', b='OK')
...
def showRealfowcache(scene):
    if plugins == None:
        cmds.confirmDialog(t=u'提示', m=u'未使用Realflow插件', b='OK')
    else:
...
def setViewport(arry):
    ...
    cmds.button(l=u'修改viewport2.0模式', c=setVP2mode)
    cmds.button(l=u'关闭maya', c=closeMaya)


def setVP2mode(give):
    mel.eval('optionVar -iv "viewportRenderer" 1')
    mel.eval('optionVar -sv "vp2RenderingEngine" "DirectX11"')
    cmds.confirmDialog(t=u'提示', m=u'需重启maya生效', b='OK')
